...
import { requestInternalGoogleToken, requestScannerLogin } from "~/core/auth/internal-auth.api";
...
function InternalEntryContent() {
  const { user, isLoading, loginWithOAuth, loginScanner } = useAuth();
  const [mode, setMode] = useState<"google" | "scanner">("google");
  ...
  const handleScannerLogin = async () => {
    ...
    try {
      const tokenData = await requestScannerLogin({
        username: username.trim(),
        password,
      });
      loginScanner(tokenData, username.trim());
    } catch (e) {
      setError(e instanceof Error ? e.message : "Username atau password salah");
    } finally {
      setIsSubmitting(false);
    }
  };
            ...
            <div className="space-y-2">
              ...
              <p className="text-sm leading-6 text-text-secondary">
                {mode === "google"
                  ...
                  : "Masuk sebagai petugas scan menggunakan username dan password yang diberikan admin."}
              </p>
            </div>
          </div>
          ...
          {mode === "google" ? (
            ...
            >
              ...
              <Button type="submit" variant="primary" size="lg" fullWidth disabled={isSubmitting}>
                {isSubmitting ? "Memproses..." : "Masuk sebagai Scanner"}
              </Button>
