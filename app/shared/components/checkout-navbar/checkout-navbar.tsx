...
export function CheckoutNavbar({ currentStep, className = "" }: CheckoutNavbarProps) {
  return (
    <header className={`sticky top-0 z-50 border-b border-border-default bg-surface-primary ${className}`}>
      <nav className="mx-auto flex h-20 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
        ...
        <div className="hidden flex-1 items-center justify-center px-8 md:flex">
          <ol className="flex w-full max-w-2xl items-center gap-2">
              ...
              return (
                <li key={step.id} className="flex flex-1 items-center last:flex-none">
                  <div className="flex items-center gap-3">
                    <span
                      className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full border-2 text-sm font-bold transition-colors ${
                        isActive
                          ? "border-brand-primary border-brand-primary text-brand-primary shadow-[0_0_10px_rgba(109,92,255,0.3)]"
                          : isCompleted
                  ...
                  {index < STEPS.length - 1 && (
                    <div
                      className={`mx-4 h-[2px] flex-1 min-w-[20px] rounded-full transition-colors ${
